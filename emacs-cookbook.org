...
*** 文件保存
   ...
   行关联
*** 读取文件内容
   将文件内容拷贝到当buffer，可以使用 *insert-file-contents* 函数
** 文件基本信息函数
  下面介绍一些与文件基本信息相关的函数
  1. 文件是否存在
  #+BEGIN_SRC emacs-lisp
...
** 互斥锁(Mutexes)
  [[https://www.gnu.org/software/emacs/draft/manual/html_node/elisp/Mutexes.html][互斥]]一种排它锁(exclusive lock)，在任何时刻，最多只允许一个线程持有互斥锁。也就是说，当一个线
