...
多少次bookmark后会自动保存一次。极端的情况，设置这个变量的值为1，这样每设置一个
bookmark，它就会自动保存一次（这样可以防止emacs突然crash时bookmark的丢失）。如
果这个值设置为nil，表示emacs不会主动保存bookmark，除非用户手动调用
*M-x bookmark-save* 。
