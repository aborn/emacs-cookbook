...
** interrupt-process采用列表interrupt-process-functions来实现传递SIGINT信号决定哪个函数会被调用。这可使Tramp实现向远程异步进程发送SIGINT信号。老的实现方式被迁移到internal-default-interrupt-process。
...
** Emacs开放了GnuTLS的加密接口(API)，其对应的函数为gnutls-macs和gnutls-hash-mac; gnutls-digests和gnutls-hash-digest; gnutls-ciphers和gnutls-symmetric-encrypt和gnutls-symmetric-decrypt

** 函数gnutls-available-p返回在Emacs中支持的GnuTLS库的功能列表

** Emacs可通过新函数make-record、record和recordp来实现用户定义类型的记录。记录用于描述cl-defstruct和defclass的实例。例如当你的代码里定义了新的记录类型，使用package-naming去命名这些类型，从而且避免类型冲突。
