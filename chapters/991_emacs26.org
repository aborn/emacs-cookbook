...
** Emacs支持在Buffer里显示行号(可选项)
   ...
   *display-line-numbers-type* 的值会与 *display-line-numbers* 的值相同。

   行号不会显示在所有的minibuffer的窗口以及提示窗口，因为他们不是用于此处。
...
*** *term-char-mode* 模式下的buffer默认为只读模式
    ...
    可以通过 *term-char-mode-buffer-read-only* 和
    *term-char-mode-point-at-process-mark* 这两个变量的值来控制, 其默认值不是nil。如果
    你想切换到老版本的行为，设置这些值为nil就可以了。
